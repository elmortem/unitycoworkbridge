...
Прогон дорог (компиляция, domain reload, для PlayMode — вход в Play Mode). Запускай тесты как финальную проверку завершённого этапа работы, а не после каждой правки.
...
- Файлы `testresult_*` чистит Bridge автоматически вместе с таском (авто-очистка последних N успешных и команда `clean.command`). Даже если результат теста записался уже после того, как таск был вычищен (осиротевший `testresult_*`) — Bridge подметает такие файлы свипом «сирот» на простое и при любой ручной очистке, так что они больше не копятся. Отдельно их не трогай.
