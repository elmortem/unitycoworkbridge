# Шаблон UNITYAGENT.md

## Зачем нужен этот файл

Unity Agent Bridge выполняет задачи пользователя через автоматически генерируемые C# скрипты. ИИ генерирует статический класс с асинхронным методом `Run()`, который запускается внутри Unity Editor:
...
`Run()` обязан возвращать `Task<string>` — мост вызывает его, не блокируя editor-поток, и пишет результат только после завершения задачи, поэтому внутри можно свободно `await`-ить async API.

Перед генерацией скрипта ИИ ищет в проекте все файлы `UNITYAGENT.md` и читает их. Если описанный в файле API подходит для задачи пользователя — ИИ использует его вместо стандартного Unity Editor API.

**Ваша задача как автора библиотеки** — написать `UNITYAGENT.md`, который объяснит ИИ:
...
Ниже — пример `UNITYAGENT.md` для гипотетического API генерации тайлмапов:
