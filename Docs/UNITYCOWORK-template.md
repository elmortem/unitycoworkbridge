...
Unity Cowork Bridge выполняет задачи пользователя через автоматически генерируемые C# скрипты. ИИ генерирует статический класс с асинхронным методом `Run()`, который запускается внутри Unity Editor:
