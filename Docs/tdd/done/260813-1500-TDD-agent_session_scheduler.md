...
- **Префлайт сцен сохранён для всех типов задач, кроме `release`.** План оставлял его только для `csharp`, `ui`, `tests` и тем самым снимал его с `compile` и `sceneshot`, хотя обе задачи умеют дотянуться до save-диалога. Восстановление контекста добавлено отдельным шагом после префлайта: `needsScenes && !HolderContextRestored` → `RestoreContext`. Для `release` префлайт пропущен намеренно — иначе dirty-сцена не давала бы отдать lease.
- **`release` от чужой сессии не вызывает ротацию.** В `TryPick` добавлен ранний выбор `release`-задачи, чья сессия не является держателем: она выполняется с `holderChanged = false` и отвечает `not_holder`. По исходному плану такая задача забирала lease у простаивающего держателя и отвечала `released`, то есть чужой `release` мог выбивать сессию из её слайса.
- **Статус очереди обновляется и во время активной задачи** (`RefreshQueueStatus`, раз в 2 секунды). Скан-цикл спит, пока задача выполняется, поэтому без этого клиент, вставший в очередь за уже идущей задачей, никогда не видел свою позицию — только `queued <n>s`.
- **Клиент не ждёт час неизвестную задачу.** Если журнальной записи нет и в inbox нет `<id>.task.json`, `WaitForTaskAsync` немедленно возвращает `task_not_found` (код 3) вместо расходования потолка очереди.
- **Отклонённый файл задачи выбывает из очереди** (`_rejectedTaskHashes` в `TaskCoordinator`). Битый `task.json` и `id_conflict` раньше выбирались планировщиком на каждом скане; в многоагентном режиме это блокировало бы уже не только своего клиента, но и все остальные сессии.
- **Контекст простоя сохраняется после снятия lease по таймауту**, а не до: порядок на результат не влияет, `SaveContextFor` не зависит от текущего держателя.
