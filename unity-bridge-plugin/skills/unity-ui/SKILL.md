...
В одном редакторе может работать несколько агентов сразу, и мост делит его между ними честно — но только если ты называешь свою сессию.

- Один раз в начале работы сгенерируй id сессии: `AB_<дата-время>_<random>` — и передавай его каждой команде: `agentbridge ui <файл> --session <id>`.
