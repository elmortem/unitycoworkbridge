...
- При `coordination-v1` агент берёт окно `--kind editor` с шагом `Kind: "ui"` и `PayloadSha256` своего `*.ui.json`, затем передаёт `--coord-window <token> --coord-step <id>`. Ручная задача без токена разрешена и ждёт освобождения окна. `--wait` включает очередь. Блокирующую несогласованную задачу можно адресно снять через `agentbridge cancel <TaskId>`; правила описаны в скилле `unity-bridge`. Писать в служебные файлы моста нельзя.
