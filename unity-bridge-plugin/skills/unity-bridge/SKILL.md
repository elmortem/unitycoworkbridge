...
Человек может запускать команды без сессии и токена даже при наличии регистраций: они ждут освобождения согласованного окна. Агенты должны брать окно для согласованной работы:

1. `agentbridge coord register --session S --spec <ТДД> --repo <корень> --scope scope.json --owner <адрес>` — резервирует область входных данных Unity. Для записи в неё нужен edit grant; работа вне этой границы остаётся свободной.
2. Перед пакетом изменений входных данных Unity — `coord edit-begin --session S --request <uuid> --seconds 120`; после завершения этих записей — `coord edit-end --session S --token <token>`.
3. Для проверок в Unity — `coord request --session S --request <uuid> --kind validation --plan plan.json`, затем `coord wait --after <revision> --wait 30`, пока не придёт `granted` с токеном. Подготовь скрипты, JSON и параметры до запроса окна. `coord wait` возвращается при изменении состояния: используй его вместо сна между опросами; после выдачи окна подавай готовые команды.
4. Рабочие команды внутри окна: `--coord-window <token> --coord-step <id>`. Шаг тратится ровно один раз, включая ответ из кэша.
5. `coord finish --session S --token <token>` — когда все задачи окна терминальны.

`scope.json`: `{ "Paths": ["UnityProject/Assets/Game/Core/"] }`, пути от корня репозитория, каталог с конечным `/`, без масок. `plan.json`: `{ "Steps": [{ "Id": "V1", "Kind": "tests", "Mode": "EditMode", "Tests": ["MyTests"], "Fresh": false }], "ArtifactRoots": [], "FixtureRoots": [] }`. Окно `validation` разрешает `compile`, `tests`, `sceneshot`; окно `editor` — `csharp`, `ui`, `sceneshot`, `compile`. Плей мод в план не входит.
...
1. Покажи кратко причину ожидания. После завершения задач своего окна освободи его через `coord finish`; свой edit grant заверши через `edit-end`, когда писатели остановлены. Не удерживай окно, необходимое владельцу ошибки для исправления.
...
4. Если Unity занята или писатели ещё меняют её входные данные — отложи повторный compile и продолжай независимую работу. Когда можно проверять, получи новое окно при активной координации и выполни один обычный `compile`. При неизменных ошибках он использует кэш. По новой диагностике повтори цикл; не переходи на `--fresh`.
