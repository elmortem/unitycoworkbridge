...
Когда протокол доступен и в проекте есть регистрации, рабочая команда без токена получает `rejected` с `coordination_required` — это не поломка, а просьба взять окно:
