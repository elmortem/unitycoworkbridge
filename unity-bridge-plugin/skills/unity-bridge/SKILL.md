...
## Координация правок (coordination-v1)

Сессионная очередь выстраивает команды, но не файловые записи. Если несколько агентов правят один проект, включается отдельный протокол. **Он существует не всегда — сперва проверь:**
...
1. `agentbridge coord register --session S --spec <ТДД> --repo <корень> --scope scope.json --owner <адрес>` — резервирует область. Писать это ещё не разрешает.
2. Перед пакетом правок — `coord edit-begin --session S --request <uuid> --seconds 120`; после того как твои писатели закончили — `coord edit-end --session S --token <token>`.
3. Для проверок — `coord request --session S --request <uuid> --kind validation --plan plan.json`, затем `coord wait --after <revision>`, пока не придёт `granted` с токеном.
...
`scope.json`: `{ "Paths": ["Game/Core/", "Docs/CORE.md"] }`, пути от корня репозитория, каталог с конечным `/`, без масок. `plan.json`: `{ "Steps": [{ "Id": "V1", "Kind": "tests", "Mode": "EditMode", "Tests": ["MyTests"], "Fresh": false }], "ArtifactRoots": [], "FixtureRoots": [] }`. Окно `validation` разрешает `compile`, `tests`, `sceneshot`; окно `editor` — `csharp`, `ui`, `sceneshot`, `compile`. Плей мод в план не входит.

Увидел `pause_requested` — закончи текущий пакет правок и вызови `edit-end`. Доводить всё ТДД до конца не нужно; если код ещё не собирается, укажи это блокером. Молчание не заменяет `edit-end`.
...
3. Подожди примерно 120 секунд без задачи в Unity — средствами ожидания хоста, при необходимости короткими интервалами. Затем прочитай `agentbridge status` и доступный статус координации. `CompilationState`, `LastCompileId`, `LastCompileFinishedUtc` описывают последний наблюдавшийся цикл, а не доказывают компилируемость текущих файлов.
4. Если Unity занята или писатели ещё работают — продолжай ожидание. Когда можно проверять, получи новое окно при активной координации и выполни один обычный `compile`. При неизменных ошибках он использует кэш. По новой диагностике повтори цикл; не переходи на `--fresh`.
