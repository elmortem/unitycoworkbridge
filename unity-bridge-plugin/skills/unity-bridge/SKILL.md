...
- Вход в плей мод не крадёт фокус: и `play`, и `tests --mode PlayMode` гасят автофокусировку Game View, а сессия идёт с `Application.runInBackground` — игра тикает, даже пока Unity в фоне. Если редактор всё же вылез вперёд, мост возвращает фокус прежнему окну (не больше двух раз за вход, чтобы не спорить с человеком, который сам кликнул в Unity). Значит, не нужно ни предупреждать пользователя, ни просить его не трогать мышь: работай как обычно.
