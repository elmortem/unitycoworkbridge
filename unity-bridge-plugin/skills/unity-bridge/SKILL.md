...
**Все тесты — только с весомой причиной.** Не запускай весь набор просто так: в некоторых проектах тестов очень много и они очень долгие, а полный прогон держит редактор для всех остальных сессий. Выбирай тесты, относящиеся к твоей правке, через `--test`, `--category` или `--assembly`. Прогон без единого фильтра CLI отклоняет до обращения к редактору с `all_tests_confirmation_required` (код `3`) и переспрашивает, точно ли нужен весь набор. Повторяй команду с `--confirm-all` только при реальной причине — например, пользователь сам попросил полный прогон; иначе сузь выборку.
...
- При capability `long-running-tasks-v1` задачи и тесты не имеют глобального лимита исполнения. Первые 300 секунд от начала исполнения защищены от чужой отмены; время очереди не считается. После 300 секунд можно снять мешающий тест или кооперативный C# через `agentbridge cancel <TaskId> --session <твоя-сессия>` без опроса владельца и отдельного разрешения пользователя, даже внутри действующего окна. Смотри `ActiveTaskElapsedSeconds` и `ActiveTaskCancelableByOtherAgents` в `status`; возраст перепроверяет сам мост. Владелец с той же непустой сессией и человек через редактор могут отменять раньше. Пустая сессия не доказывает владение. Ответ `cancel_requested` означает принятую отмену: дождись адресного результата через `wait`. При `cancel_blocked` после 30 секунд сообщи о блокировке и продолжай доступную работу вне Unity. Не удаляй очередь и не завершай процесс. Тестовый прогон, чей Unity-джоб оборвался без результатов, отменять не надо: он сам завершается `runtime_error` не позже чем через 25 секунд, и очередь идёт дальше. На старом пакете без этой capability действует прежняя политика; обнови пакет для длинных прогонов.
...
Для нового порядка записи CLI и пакет должны объявлять `coordination-edit-leases-v1` (CLI ≥ 1.22.0, пакет ≥ 0.31.0). Обновляются оба компонента: новый CLI откажется менять координацию старого пакета. При первой мутации состояние автоматически переходит на schema 2, которую старый CLI не обслуживает. Регистрация — декларация намерений, а не владение файлами: пересечения разрешены, idle-сессия и ожидание тестов ничего не бронируют. Не снимай чужую регистрацию ради пересечения.
...
2. Для их записи: `coord edit-begin --session S --request <uuid> --seconds 120`. При `waiting` жди выдачи права через coord wait/status по той же заявке; писать можно только после `granted` с токеном. После выдачи перечитай актуальные файлы: между заходами их мог изменить другой агент. Затем `coord edit-end --session S --token <token>`, когда записи завершены. Независимая работа разрешений моста не требует.
...
Пересекающиеся записи идут по очереди; непересекающиеся могут выполняться вместе. Срок edit grant — 15–300 секунд от выдачи, по умолчанию 120. Ожидающее окно или пересекающийся писатель запрещает продление (`pause_requested`): заверши текущую запись и вызови `edit-end`. После дедлайна право освобождается автоматически; `renew` старого токена вернёт `stale_token`, поздний `edit-end` — `already_closed`. Для продолжения получи новое право с новым UUID и перечитай файлы. Не оставляй фоновые процессы записи за пределами срока. Scope нельзя менять с активным правом или ожидающей заявкой.

Обрывы различаются: пропавший клиент и истёкший `wait` не отменяют задачу Unity, domain reload от своего compile/PlayMode сохраняет окно и шаги, перезапуск редактора закрывает окно после восстановления. Выполняющаяся задача сохраняет окно до завершения/отмены и восстановления; дедлайн не делает редактор свободным. Старые `orphaned` edit grants освобождаются автоматически при обновлении. `coord abandon --target-session S --reason "<текст>"` остаётся аварийным снятием ещё живого права по явному решению пользователя после подтверждения остановки писателей; для idle-регистраций и протухших прав он не нужен.
...
Свидетелей у прогона два: файловый наблюдатель и stat-манифест входов (путь, размер, mtime), который переживает domain reload в файле. Поэтому PlayMode-прогон замечает изменение с возвратом и в разрыве наблюдения, который создаёт сам себе; в `Reason` успешного такого прогона стоит `the stat manifest covers the gap`. Потерянный или нечитаемый манифест даёт `unknown`, а не `valid`.

Динамические `TMP_FontAsset` целиком исключены из хеша и файлового наблюдения: сохранение глифов и встроенных атласов не сбрасывает кеш. Статические шрифты, исходные TTF/OTF, `.meta` и отдельные текстуры/материалы остаются под проверкой. После ручных правок внутри динамического font asset используй `tests --fresh`; режим Static возвращает файл под контроль при следующем снимке.
...
`runtime_error` тестового прогона с `ended without RunFinished` или `Unity test run failed:` в логах — это сбой самого Test Framework, а не падение тестов. Исправлять нечего: перезапусти прогон.
...
У пакета с `long-running-tasks-v1` глобальный таймаут исполнения убран; старый `TaskTimeoutSeconds` игнорируется. `timeout` может относиться к специализированной операции или старой версии пакета: читай причину. Для C# отмена кооперативная: принимай `CancellationToken`, проверяй его в циклах и передавай в ожидания. До фактической остановки и восстановления сцен редактор остаётся занят. Истечение окна координации запрещает новые шаги, но не прерывает уже выполняющийся. Отдельный тест сохраняет собственный лимит NUnit: в Unity Test Framework 1.1.33 по умолчанию 180 секунд. Для намеренно долгого теста укажи `[Timeout(1200000)]` (миллисекунды) или другой подходящий срок. Bridge не переопределяет этот лимит. В 1.1.33 его превышение может заблокировать редактор синхронным дочитыванием оставшегося IEnumerator; исправляй лимит теста, не повторяй такой прогон без исправления.

### `Status == "canceled"`

Задачу отменил владелец, человек или другой агент после 300 секунд. При `preempted_after_300s` журнал и результат сообщают инициатора и длительность. Сообщи пользователю причину; это прерванная проверка, не успешная приёмка. Не перезапускай снятый пакет немедленно: его оставшиеся шаги прекращены, редактор нужен соседу. После ручной отмены не перезапускай без явного решения пользователя.
...
Выдача из кеша доказывается как прогон: дайджест входов сверяется дважды, вокруг обоих замеров стоит stat-манифест, и правка входа с возвратом во время проверки не даёт `Cached: true`. Пока подходящей записи в кеше нет, дайджест не считается вовсе; устойчивый промах пересчитывается не чаще раза в 10 секунд, а правка исходников или новая запись в кеше проверяется сразу. Ждущая задача от этого не дешевеет и не дорожает — просто ожидание в очереди не занимает редактор фоновой работой.
