...
- Плей мод без сессии (застрявший таск, ручной запуск) гасит `stopplay` от любого агента. Больше того, ничейный плеймод CLI останавливает сам, когда за ним ждёт твоя задача, — специально подавать `stopplay` не нужно. Вне плей мода `stopplay` — no-op со `success` и `ReturnValue: "not_playing"`.
